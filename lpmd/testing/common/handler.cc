...
{
 scell = NULL;
 GlobalSession.DefineKeyword("debug", "none"); 
 GlobalSession.DefineKeyword("distancecache", "false");
}
...
void CommonHandler::Initialize()
{
 ...
 if ((GlobalSession["debug"] != "none") && (! param.GetBool("replacecell")))
 {
  GlobalSession.DebugStream() << "Cell vectors: " << '\n';
  for (int k=0;k<3;++k) GlobalSession.DebugStream() << GetCell().GetVector(k) << '\n';
  GlobalSession.DebugStream() << "Volume: " << GetCell().Volume() << '\n';
 }
...
{
 PluginManager pm;
 BannerPrint("Plugin: "+plugin);
 pm.LoadPlugin(plugin, "tempmodule", "");
 std::cout << "  Plugin name          : " << pm["tempmodule"].Name() << '\n';
 std::cout << "  Plugin version       : " << pm["tempmodule"]["version"] << '\n';
 std::cout << "  LPMD API required    : " << pm["tempmodule"]["apirequired"] << '\n';
 std::cout << "  Loaded from file     : " << pm["tempmodule"]["fullpath"] << '\n';
 std::cout << "  Report bugs to       : " << pm["tempmodule"]["bugreport"] << '\n';
 BannerPrint("Usage");
...
void CommonHandler::Execute(CommonCmdLineParser & clp)
{
 ...
 if (clp.Defined("verbose")) GlobalSession.AssignParameter("debug", "stderr");
 ...
 {
  if (GlobalSession["debug"] != "none") GlobalSession.DebugStream() << "-> Reading input file: \"" << clp["inputfile-file"] << "\"\n";
 ...
 if (GlobalSession["debug"] != "none") GlobalSession.DebugStream() << GlobalSession << std::endl;
